...
void find_farthest_target(int *tree, vector<int> *target, int index, int previous, int N, int current_step, int* max_step);
void find_target_array(int flag, int N, int *tree);
...
{   
    ...
    cin >> FlagNodeValue;
    find_target_array(FlagNodeValue,nodes,tree);
...
{   
    ...
    {   
        find_farthest_target(tree,target,left_child_index,tree[index],N,current_step+1,max_step);
    }

    if(right_child_index<N && tree[right_child_index]!=-1 && tree[right_child_index]!=previous)
    {
        find_farthest_target(tree,target,right_child_index,tree[index],N,current_step+1,max_step);
    }

    if(0<=parent_index && parent_index<N && tree[parent_index]!=-1 && tree[parent_index]!=previous)
    {
        find_farthest_target(tree,target,parent_index,tree[index],N,current_step+1,max_step);
    }
}
...
void find_target_array(int flag, int N, int *tree)
{
    ...
    find_farthest_target(tree,&targets,current_node_index,previous,N,current_step,&max_step);
